#-------------------------------------------------------------------------------
# Name:        Database Sync & Publiction Script - CARTA PUBLISH BACKSYNC
...
#               skipping steps in the process.  Part of fix was removing
#
#-------------------------------------------------------------------------------
...
db_connection = r'Database Connections\\Connection to WebGIS on SQL2016STG.sde'
...
def check_if_update (db_connection):
    ...
    data_source = db_connection + '\\ADMINGTS.View_LIS_Layers_To_Update'
...
def check_for_existance(conn_string, target_db, item_to_check):
    ...
    if arcpy.Exists(item_check):
        ...
        try:
            ...
            print (arcpy.AddError("Error Messages: " + arcpy.GetMessages()))
            print "Leaving Check For Existing Layer----"
...
def copy_layer_over (input_connection, output_connection, target_db, pub_layerfullname_add):

    print "Entering Copy Layer----"

    # Configure Connections
    input_connection = input_connection + '\\' + current_db + '.' + pub_layerfullname
    output_connection = output_connection + '\\' + target_db + '.' + pub_layerfullname_add
    ...
    try:
        ...
        print "Layer has not been deleted.  Please remove lock and try again."
...
if updatecnt > 0:
    ...
    check_pending_updates_sql = '''select [Data_Owner], [Table_Name], [SDE_Name], [Type], CONVERT(VARCHAR(8), [Date_Last_Modified], 1) AS [Date_Last_Modified] from [ADMINGTS].[View_LIS_Layers_To_Update]'''
    ...
    for row in check_pending_updates_return:
        ...
        pub_layerfullname_add = 'GIS.' + fc_update_table
            ...
            if pub_published == 'Y':
                ...
                if pub_scratchdb <> None:
                    ...
                    try:
                        ...
                        item_to_check = pub_layerfullname_add
                    ...
                    target_db_type = pub_targetdb_type
                    obtain_dbase_connection(db_connection, target_db, target_db_type, fc_update_owner)
                    conn_string = 'Database Connections\\GIS connection to CartaPublish on SQLCLUSTERGIS.sde'
                    ...
                    copy_layer_over (input_connection, output_connection, target_db, pub_layerfullname_add)

                    print pub_layerfullname_add + " has successfully been published to its target destination of " + target_db + " on " + pub_targetdb_type + "."
